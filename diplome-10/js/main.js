...
questionsItem.forEach((el) => {
  el.addEventListener("click", (even) => {
    // let itemAll = document.querySelectorAll(".questions__item");

    // itemAll.forEach((ia) => {
    //   if (ia.classList.contains('questions__item-opend')) {
    //     ia.classList.remove("questions__item-opend");
    //   }
    // })

    // if (even.target.classList.contains('questions__control-text')) {
    //   even.target = even.target.parentElement;
    // }
    ...
    let opend = document.querySelector(
      ".questions__item-opend .questions__item"
    );
